...
namespace VS
{
    ...
    */
    ...
    String &String::append(const String::const_iterator first, const String::const_iterator second)
    {
        ...
        if (n != 0)
        { // if length is not zero then _append
            char *buffer = nullptr;
    ...
    */
    ...
    size_t String::find_first_not_of(char c, size_type pos) const
    { // character(4)
    ...
    size_t String::find_last_not_of(const char *other, size_type pos) const
    { // c - string(2)
    ...
    */
    ...
    {
        if (_str_len == n)
            return; // length  already allocated
        if (_str_len < n)
        {
            _setLength(n);
            return;
